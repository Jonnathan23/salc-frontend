...
import { mockStudents, mockSessions, mockAlerts } from '@/lib/classtrack-mock-data';
import { SessionStatus, AlertStatus, ContractStatus, ProgressCategory } from '@/lib/classtrack-types';
import { ContractStatusBadge, ProgressCategoryBadge } from './shared/Badges';

export function DashboardView() {
    const studentsInside = mockSessions.filter(
        (s) => s.attendanceSessionStatus === SessionStatus.IN_PROGRESS
    ).length;

    const pendingCheckouts = mockSessions.filter(
        (s) => s.attendanceSessionStatus === SessionStatus.PENDING_APPROVAL
    ).length;

    const activeAlerts = mockAlerts.filter(
        (a) => a.retentionAlertStatus === AlertStatus.PENDING
    ).length;

    const activeStudents = mockStudents.filter(
        (s) => s.studentContractStatus === ContractStatus.ACTIVE
    ).length;
                        ...
                        {recentSessions.map(({ session, student }) => (
                            <div key={session.attendanceSessionId} className="px-5 py-3 flex items-center justify-between">
                                ...
                                <div className="text-right">
                                    <p className="text-xs font-semibold text-[var(--color-quinary)]">Desde {session.attendanceSessionEntryTime}</p>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="bg-[var(--color-primary)]/20">
                                ...
                                <th className="px-5 py-3 text-left text-xs font-semibold text-[var(--color-quinary)] uppercase tracking-wide">Cedula</th>
                                <th className="px-5 py-3 text-left text-xs font-semibold text-[var(--color-quinary)] uppercase tracking-wide">Certificado</th>
                                <th className="px-5 py-3 text-left text-xs font-semibold text-[var(--color-quinary)] uppercase tracking-wide">Contrato</th>
                                <th className="px-5 py-3 text-left text-xs font-semibold text-[var(--color-quinary)] uppercase tracking-wide">Progreso</th>
                            </tr>
                        </thead>
                                ...
                                <tr key={student.studentId} className="hover:bg-[var(--color-primary)]/10 transition-colors">
                                    <td className="px-5 py-3">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="font-medium text-[var(--color-font)]">{student.studentFullName}</span>
                                        </div>
                                    </td>
                                    <td className="px-5 py-3 text-[var(--color-font)]/60 font-mono text-xs">{student.studentIdentificationCard}</td>
                                    <td className="px-5 py-3 text-[var(--color-font)]/70">{student.studentCertificateType}</td>
                                    <td className="px-5 py-3"><ContractStatusBadge status={student.studentContractStatus} /></td>
                                    <td className="px-5 py-3"><ProgressCategoryBadge category={student.studentProgressCategory} /></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
