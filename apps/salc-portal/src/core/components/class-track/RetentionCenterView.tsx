...
export function RetentionCenterView() {
    ...
    const handleRowClick = (alert: RetentionAlert) => {
        ...
        setEditForm({
            contactDate: alert.retentionAlertContactDate
                ? alert.retentionAlertContactDate.toISOString().split('T')[0]
                : '',
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="bg-[#f5e0d4]/50">
                                <th className="px-5 py-3 text-left text-xs font-semibold text-[var(--color-quaternary)] uppercase tracking-wide">Estudiante</th>
                                <th className="px-5 py-3 text-left text-xs font-semibold text-[var(--color-quaternary)] uppercase tracking-wide">Contrato</th>
                                <th className="px-5 py-3 text-left text-xs font-semibold text-[var(--color-quaternary)] uppercase tracking-wide">Dias Ausente</th>
                                <th className="px-5 py-3 text-left text-xs font-semibold text-[var(--color-quaternary)] uppercase tracking-wide">Contacto</th>
                                <th className="px-5 py-3 text-left text-xs font-semibold text-[var(--color-quaternary)] uppercase tracking-wide">Estado</th>
                            </tr>
                        </thead>
                                    ...
                                    >
                                        <td className="px-5 py-3">
                                            <div className="flex items-center gap-2.5">
                                                ...
                                                <div>
                                                    <p className="font-medium text-[var(--color-font)]">{student?.studentFullName}</p>
                                                    <p className="text-xs text-[var(--color-font)]/50">{student?.studentPhoneNumber}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-5 py-3">
                                        ...
                                        <td className="px-5 py-3">
                                            <span className={`font-bold text-lg ${alert.retentionAlertDaysAbsent >= 14 ? 'text-[var(--color-quaternary)]' : 'text-[var(--color-sextary)]'}`}>
                                        ...
                                        <td className="px-5 py-3">
                                            {alert.retentionAlertContactDate
                                                ? <span className="text-[var(--color-font)] text-xs">{alert.retentionAlertContactDate.toLocaleDateString('es-VE')}</span>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="bg-[var(--color-primary)]/10">
                                    <th className="px-5 py-3 text-left text-xs font-semibold text-[var(--color-quinary)] uppercase tracking-wide">Estudiante</th>
                                    <th className="px-5 py-3 text-left text-xs font-semibold text-[var(--color-quinary)] uppercase tracking-wide">Dias Ausente</th>
                                    <th className="px-5 py-3 text-left text-xs font-semibold text-[var(--color-quinary)] uppercase tracking-wide">Justificado</th>
                                    <th className="px-5 py-3 text-left text-xs font-semibold text-[var(--color-quinary)] uppercase tracking-wide">Razon</th>
                                ...
                                {resolvedAlerts.map((alert) => {
                                    ...
                                    return (
                                        <tr key={alert.retentionAlertId} className="opacity-70">
                                            <td className="px-5 py-3 font-medium text-[var(--color-font)]">{student?.studentFullName}</td>
                                            <td className="px-5 py-3 text-[var(--color-font)]/70">{alert.retentionAlertDaysAbsent} dias</td>
                                            <td className="px-5 py-3">
                                                <span className={`text-xs font-semibold ${alert.retentionAlertIsJustified ? 'text-[var(--color-quinary)]' : 'text-[var(--color-quaternary)]'}`}>
                    ...
                    <div className="fixed inset-0 z-50 flex">
                        ...
                        <div className="w-full max-w-md bg-white shadow-2xl flex flex-col h-full overflow-y-auto">
                            ...
                            <div className="px-6 py-5 border-b border-[var(--color-tertiary)]/20 flex items-center justify-between sticky top-0 bg-white">
                                ...
                                <button onClick={() => setSelectedAlert(null)} className="p-1.5 hover:bg-gray-100 rounded-lg transition-colors">
                            ...
                            <div className="px-6 py-4 bg-[var(--color-primary)]/15">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-[var(--color-font)]/60">{student?.studentIdentificationCard}</p>
                            ...
                            <div className="flex-1 px-6 py-5 space-y-5">
                                ...
                                <div className="flex items-center justify-between p-3 bg-[var(--color-primary)]/10 rounded-lg">
                                    ...
                                    >
                                        <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${editForm.hasResponded ? 'left-5' : 'left-0.5'}`} />
                                    </button>
                                </div>
                                ...
                                <div className="flex items-center justify-between p-3 bg-[var(--color-primary)]/10 rounded-lg">
                                    ...
                                    >
                                        <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${editForm.isJustified ? 'left-5' : 'left-0.5'}`} />
                                    </button>
                                </div>
                                ...
                                {editForm.isJustified && (
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            onChange={(e) => setEditForm({ ...editForm, justificationReason: e.target.value })}
