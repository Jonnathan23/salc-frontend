...
import { useIsMobile } from '@/core/components/ui/use-mobile'
...
function SidebarMenuButton({ asChild = false, isActive = false, variant = 'default', size = 'default', tooltip, className, ...props }: SidebarMenuButtonProps) {
    const Comp = asChild ? Slot : 'button'
...
function SidebarMenuSkeleton({ className, showIcon = false, ...props }: SidebarMenuSkeletonProps) {
    const [sidebarWidth, setSidebarWidth] = useState<string>("50%")
