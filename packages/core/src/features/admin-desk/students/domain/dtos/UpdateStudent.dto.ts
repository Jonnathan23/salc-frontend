...
export class UpdateStudentDtoImpl implements UpdateStudentDto {
    ...
    static create(data: Record<string, any>): UpdateStudentDto {
        ...
        if (identificationCard && !Validators.isIdentificationCard(identificationCard))
            throw CustomError.badRequest("Invalid identificationCard");
        ...
        if (startDate && !dateRegularExpression.test(startDate))
            throw CustomError.badRequest("Invalid startDate format. Expected YYYY-MM-DD");
        if (dateOfBirth && !dateRegularExpression.test(dateOfBirth))
            throw CustomError.badRequest("Invalid dateOfBirth format. Expected YYYY-MM-DD");

        if (nationality && nationality.length < 3) throw CustomError.badRequest("Invalid nationality");
        if (certificateType && !Validators.isCertificateType(certificateType))
            throw CustomError.badRequest("Invalid certificateType");
        if (contractStatus && !Validators.isStudentContractStatus(contractStatus))
            throw CustomError.badRequest("Invalid contractStatus");
