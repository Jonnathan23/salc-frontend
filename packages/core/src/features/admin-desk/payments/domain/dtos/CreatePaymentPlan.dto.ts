...
export class CreatePaymentPlanDtoImpl implements CreatePaymentPlanDto {
    ...
    static create(data: Record<string, any>): CreatePaymentPlanDto {
        ...
        if (data.enrollmentFee === undefined || data.enrollmentFee === null || isNaN(data.enrollmentFee))
            throw CustomError.badRequest("enrollmentFee must be a valid number");
        if (data.totalAmount === undefined || data.totalAmount === null || isNaN(data.totalAmount))
            throw CustomError.badRequest("totalAmount must be a valid number");
        if (data.isSinglePayment === undefined) throw CustomError.badRequest("isSinglePayment is required");
        if (data.numberOfQuotas === undefined || data.numberOfQuotas === null || isNaN(data.numberOfQuotas))
            throw CustomError.badRequest("numberOfQuotas must be a valid number");
